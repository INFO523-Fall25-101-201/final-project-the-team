...
Exploratory data analysis was conducted to understand the distribution and relationships of orbital parameters prior to modeling. A condensed version of the EDA is shown in this report.
...
The created pairwise scatter plots colored by orbit class demonstrated clear clustering of Amor, Apollo, and Aten objects in orbital parameter space, particularly when examining perihelion distance versus semimajor axis. Class imbalance was also observed across groups, with Aten objects appearing less frequently than other groups, motivating the use of stratified sampling and balanced classification approaches. The knowledge of the class imbalance, potential feature correlations, and general knowledge of the dataset allows for effective model creation and the use of models that work well with the dataset and its features.
...
Three supervised learning models were evaluated for this project. Logistic regression was first evaluated and was used to establish a linear baseline. The following model evaluated was the K-Nearest Neighbor to introduce nonlinear, distance based classification. These models were scaled appropriately using StandardScaler. Lastly, the final and chosen model was a Random Forest Classifier. This model was trained and evaluated for both orbit class and hazardous prediction task since its ability to model nonlinear feature interactions and well as handle class imbalance efficiently. The models were put into a generic pipeline with no or minimal hyperparameter selections outside the defaults. The goal was to get baseline performance prior to getting the optimized model. Base models also run quickly, and cross validation could add computational complexity, which is why none was performed until the final model was selected.

The models were evaluated using with a mixture of metrics with accuracy, F1 score, ROC AUC, as well as confusion matrices. The combination of factors, including hyperparameter tuning of the best models, allows for informed decision-making and understanding the models' true capabilities for predicting NEOs as hazardous or which group they belong to. Below shows most of the modeling process, but only the tuned Random Forest models are shown to keep outputs simple.
...
This project assessed three models for classifying Near-Earth Objects based on orbital features. Logistic Regression performed well with 90.83% accuracy and an F1 score of 0.9, while KNN followed with 88.07% accuracy and an F1 score of 0.87. The Random Forest Classifier achieved an impressive tuned ROC AUC of 0.93 and a tuned f1-score of 0.93 and clearly outperformed the baseline models. Its feature importance rankings identified MOID, absolute magnitude, and key orbital elements such as q, e, and Q as the most influential predictors, consistent with known NEO dynamical behavior. Overall, the Random Forest provided the most accurate and physically interpretable model for this task. To further expand the Random Forest's capabilities, this model achieved a tuned 90% accuracy, tuned .907 ROC-AUC in predicting the NEO group to which the objects belong. To account for any potential overfitting of the models, hyperparameter tuning was performed on the final models. This process lowered the accuracy and ROC-AUC, but the models still perform incredibly well at accuracy and ROC-AUC. These experiments show that using the Random Forest model can be extended to explore live APIs from NASA to quickly determine risk and object type. The results of this project demonstrate the final models' abilities to predict and group NEOs to their appropriate group and hazardous status.